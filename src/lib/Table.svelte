      ...
      <div class="flex items-center justify-between py-3">
        <form method="POST" action="/profiles?/update">
          <div class="flex items-center space-x-4">
            <div class="flex">
              <input class="font-medium pt-1 leading-none " name="name" id = "name" value={user.name} size={user.name.length} 
			  style="width: auto;"/>
              <input class="font-medium pl-5 text-gray-500 pt-0 " name="email" id="email" value ={user.email} size={user.email.length} 
			  style="width: auto;" />
			  <input type="hidden" name="id" id="id" value={user.id}/>
			  <button class=" ml-4 " type="submit">Update</button>
            </div>
          </div>
        </form>
